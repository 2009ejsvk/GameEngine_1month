...
namespace
{
    ...
    FAlmanacSnapshot BuildSnapshot(
        ...
        const std::shared_ptr<CMainWorld>& MainWorld);
...
namespace
{
        ...
        const std::shared_ptr<CMainWorld>& MainWorld)
    {
        ...
        {
            const double Denominator =
                static_cast<double>(Snapshot.PoliticalSnapshot.ActiveCitizenCount);
        ...
        if (Snapshot.MartialLawActive)
            Snapshot.RebelRiskScore = (std::max)(0.0, Snapshot.RebelRiskScore - 8.0);
...
void CAlmanacWidget::RefreshData()
{
    ...
    const FAlmanacSnapshot Snapshot = BuildSnapshot(World, MainWorld);
