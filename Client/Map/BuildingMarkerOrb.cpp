    ...
    float DeltaTime)
{
    ...
    const int TileIndex = TileMap->GetTileIndex(Mesh->GetWorldPos());
    ...
    std::vector<std::string> UniqueNames;
    UniqueNames.reserve(TargetNames.size());
    ...
    bool IsOccludedByBuilding = false;

    for (size_t i = 0; i < UniqueNames.size(); ++i)
    {
        auto Building =
            World->FindObject<CPlacementAreaObject>(UniqueNames[i]).lock();

        if (!Building)
            continue;

        if (Building->ContainsBackOcclusionTileIndex(TileIndex))
        {
            IsOccludedByBuilding = true;
            break;
        }
    }

    // 알파 페이드: 목표값으로 부드럽게 전환
    const float TargetAlpha = IsOccludedByBuilding ? 0.f : 1.f;
    const float FadeStep = mOcclusionFadeSpeed * DeltaTime;

    if (mOcclusionAlpha < TargetAlpha)
    {
        mOcclusionAlpha += FadeStep;

        if (mOcclusionAlpha > TargetAlpha)
            mOcclusionAlpha = TargetAlpha;
    }

    else if (mOcclusionAlpha > TargetAlpha)
    {
        mOcclusionAlpha -= FadeStep;
    ...
    Mesh->SetMaterialOpacity(0, mOcclusionAlpha);
    Mesh->SetRenderLayer("MarkerOrb");
    Mesh->SetEnable(mOcclusionAlpha > 0.01f);
}
