...
DEFAULT_OUTPUT_TSV = Path(
    r"E:\GameEngine_1month\Client\Building\Data\Tropico6CostOverrides.tsv"
)


MATCHES = [
    ("build_1_3", "DA_T6DockBP"),
    ("build_1_4", "DA_Building_ConstructionOfficeBP"),
    ("build_1_5", "DA_Building_TeamsterBP"),
    ("build_1_6", "DA_T6LandingBP"),
    ("build_1_7", "DA_T6StorageQuayBP"),
    ("build_1_8", "DA_T6Warehouse_BuildingBP"),
    ("build_1_13", "DA_BusStop"),
    ("build_1_14", "DA_Tunnel"),
]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate Tropico 6 cost override data for the runtime building catalog."
    )
...
def normalize_cost(text: str) -> str:
    if not text:
        return ""
    try:
...
def main() -> None:
    ...
    output_lines = [
        "# BuildingId\tBlueprintCost\tConstructionCost\tSourceAssetName\tSourceDisplayName"
    ]
    ...
    for building_id, asset_name in MATCHES:
        ...
        output_lines.append(
            "\t".join(
                [
                    building_id,
                    normalize_cost(row.get("BlueprintCost", "")),
                    normalize_cost(row.get("BuildCost", "")),
                    asset_name,
